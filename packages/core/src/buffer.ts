...
export interface Cell {
  ...
  fg: number;     // RGBA packed as uint32
  bg: number;     // RGBA packed as uint32
  flags: number;  // Bitfield (bold / italic / underline)
}
...
export function index(buffer: Buffer, col: number, row: number): number {
  if (col < 0 || col >= buffer.width || row < 0 || row >= buffer.height) {
    throw new RangeError(
      `Cell (${col}, ${row}) out of bounds for ${buffer.width}×${buffer.height} buffer`
    );
  }
...
export function getCell(buffer: Buffer, col: number, row: number): Cell {
...
export function setCell(
  ...
  const i = index(buffer, col, row);
  if (cell.char  !== undefined) buffer.chars[i] = cell.char;
  if (cell.fg    !== undefined) buffer.fg[i]    = cell.fg;
  if (cell.bg    !== undefined) buffer.bg[i]    = cell.bg;
  if (cell.flags !== undefined) buffer.flags[i] = cell.flags;
}
